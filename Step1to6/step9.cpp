...
string buildGetResponse(const HttpRequest& req) {
    if (!isPathSafe(req.path))
        return buildHttpResponse(403, "Forbidden", "text/plain; charset=utf-8", "403 - 非法路径");
...
string do_request(const HttpRequest& req) {
    ...
    return buildHttpResponse(405, "Method Not Allowed", "text/plain; charset=utf-8",
        "暂只支持 GET 和 POST /echo");
}
...
public:
    ...
    void submit(function<void()> task) {
        { lock_guard<mutex> lock(mtx_); tasks_.push(move(task)); }
        cv_.notify_one();
    }
private:
    void workerLoop() {
...
WriteResult process_write(int epfd, HttpConnection& conn) {
    ...
    while (conn.write_idx_ < conn.write_buf_.size()) {
        ...
        ssize_t n = write(conn.fd_, conn.write_buf_.data() + conn.write_idx_, remaining);
        if (n > 0) {
...
void handle_write(int epfd, HttpConnection& conn,
    ...
    if (wr == WriteResult::Complete) {
        LOG_INFO("发完 fd=" + to_string(conn.fd_) + " 共 " +
                 to_string(conn.write_buf_.size()) + " 字节");
        closeConnection(epfd, conn.fd_, connections);
    } else if (wr == WriteResult::Error) {
        LOG_ERROR("写失败 fd=" + to_string(conn.fd_));
        closeConnection(epfd, conn.fd_, connections);
    }
}
...
void drain_ready_responses(int epfd,
    ...
    while (!local.empty()) {
        ...
        LOG_INFO("开始发送 fd=" + to_string(conn.fd_) +
                 " 响应长度=" + to_string(conn.write_buf_.size()));

        modFd(epfd, conn.fd_, EPOLLOUT);
        WriteResult wr = process_write(epfd, conn);
        if (wr == WriteResult::Complete) {
            LOG_INFO("发完 fd=" + to_string(conn.fd_));
            closeConnection(epfd, conn.fd_, connections);
        } else if (wr == WriteResult::Error) {
            closeConnection(epfd, conn.fd_, connections);
        }
...
void handle_read(int epfd, HttpConnection& conn, ThreadPool& pool,
    ...
    if (!append_read(conn)) {
        LOG_WARN("读失败或连接关闭 fd=" + to_string(conn.fd_));
        closeConnection(epfd, conn.fd_, connections);
        return;
    }
    conn.refresh_active();
    ...
    if (result == ReadResult::Error) {
        LOG_ERROR("解析错误 fd=" + to_string(conn.fd_));
        closeConnection(epfd, conn.fd_, connections);
        return;
    }
    ...
    LOG_INFO("请求完整 fd=" + to_string(client_fd) + " " +
             req.method + " " + req.path);

    pool.submit([client_fd, req]() {
        string response = do_request(req);
        LOG_INFO("[worker] 拼好响应 fd=" + to_string(client_fd) +
                 " len=" + to_string(response.size()));
        {
            lock_guard<mutex> lock(g_ready_mtx);
...
int main() {
    ...
    if (bind(listen_fd, (sockaddr*)&addr, sizeof(addr)) < 0) {
        LOG_ERROR("bind 失败"); return 1;
    }
    if (listen(listen_fd, 128) < 0) {
        LOG_ERROR("listen 失败"); return 1;
    }

    setNonBlocking(listen_fd);

    int epfd = epoll_create1(0);
    if (epfd < 0) { LOG_ERROR("epoll_create1 失败"); return 1; }
    ...
    cout << "Step9 日志服务器已启动：http://127.0.0.1:" << PORT << "\n";
    cout << "日志文件: " << LOG_PATH << "（异步=" << (LOG_ASYNC ? "开" : "关") << "）\n";
    LOG_INFO("服务器启动 端口=" + to_string(PORT));
    ...
    while (true) {
        ...
        for (int i = 0; i < nready; ++i) {
            ...
            if (fd == g_notify_fd) {
                uint64_t buf;
                while (read(g_notify_fd, &buf, sizeof(buf)) > 0) {}
