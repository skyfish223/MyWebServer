...
const size_t MAX_BODY_SIZE = 10204*1024;
...
public:
    ...
    }//difftime函数及其参数的讲解，同时告诉我为什么difftime中的有一个参数是nullptr？
};
...
void tick_expired_connections(int epfd, unordered_map<int, HttpConnection>& connections)
{
    vector<int> to_close;
...
void handle_read(int epfd, HttpConnection& conn, ThreadPool& pool, unordered_map<int, HttpConnection>& connections)
{
    if (!append_read(conn)) {
